...
export const WorkCard: FC<IWorkCard> = ({
    ...
    return (
        <li className="animate__animated animate__fadeInUp">
            <div
                className="max-w-3xl h-full flex flex-col mx-auto bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                ...
                <div className="p-5 flex-grow inline-flex flex-col">
                    ...
                    <p className="mb-auto font-normal text-gray-700 dark:text-gray-400">{description}</p>
                    <ul className="flex w-full gap-x-1 items-center flex-wrap mb-2.5">
                        {tech.map((i) => (<Tech key={i.attributes.logo.data.id} attributes={i.attributes}/>))}
                    </ul>
                    <a href={link} target="_blank"
                       className="inline-flex items-center justify-center px-5 py-3 text-base font-medium text-center text-white rounded-lg bg-purple-700 hover:bg-purple-800 focus:ring-4 focus:ring-purple-300 dark:focus:ring-purple-900 transition-colors">
                        Go to
                        <svg aria-hidden="true" className="max-w-[1.5rem] ml-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"
                             fill="currentColor">
                            <path
